<?php
/**
 * Multi_Newsletter_Signup_Form_Endpoint class file
 *
 * @package courier
 */
...
/**
 * Endpoint for multi-newsletter signup form submissions.
 */
final readonly class Multi_Newsletter_Signup_Form_Endpoint implements Feature {
	/**
	 * Constructor.
	 *
	...
	public function boot(): void {
		$this->path_dispatch->add_path(
			[
				'path'     => 'api/multi-newsletter-signup-form',
	...
	public function callback(): never {
		...
		$nonce = sanitize_text_field( wp_unslash( $this->request->request->getString( '_wpnonce' ) ) );

		if ( ! wp_verify_nonce( $nonce, 'courier_multi_newsletter_signup_form' ) ) {
			$this->outcome_logger->log(
				[
					'request_id'           => $request_id,
					'endpoint'             => 'multi',
		...
		try {
			$list_name = $this->request->request->all( 'list_name' );
			$list_name = array_filter( $list_name, is_string( ... ) );
			$list_name = array_map( fn ( string $raw ): string => sanitize_text_field( wp_unslash( $raw ) ), $list_name );
			$list_name = array_unique( $list_name );
			$list_name = array_values( $list_name );
		} catch ( Exception ) {
			$list_name = [];
		}
		...
		$form_attrs = [
			'author'       => $author,
			'formId'       => sanitize_text_field( wp_unslash( $this->request->request->getString( 'form_id' ) ) ),
			'listName'     => $list_name,
		...
		if ( ! $this->verify_recaptcha( $recaptcha_token ) ) {
			...
			$block = new Named_Block(
				block_name: 'courier/multi-newsletter-signup-form',
				attrs: [
					...$form_attrs,
		...
		$invalid_list_names = $this->get_invalid_list_names_for_signup_url( $signup_url, $list_name );

		if ( count( $invalid_list_names ) > 0 ) {
			$this->log_invalid_submission(
				'invalid_list_name',
				$signup_url,
				$referrer,
		...
		$source = 'fb_leads' === $utm_source ? 'fb_leads' : explode( '?', $signup_url, 2 )[0];

		$sailthru_attempted = false;
		$sailthru_result    = 'not_attempted';

		if ( is_email( $email ) && count( $list_name ) > 0 ) {
			try {
				$vars = [
				...
				$result             = $this->sailthru_client->saveUser(
					...
					$email,
					[
						'lists' => array_fill_keys( $list_name, 1 ),
						'vars'  => $vars,
					],
				);
		...
		if ( ! $success ) {
			...
			$form_id = sanitize_text_field( wp_unslash( $this->request->request->getString( 'form_id' ) ) );
			...
			if ( $email !== '' ) {
				...
				// Take only the form from the block output.
				header( 'HX-Reselect: form' );
			}
		}
	...
	/**
	 * Log an invalid newsletter signup submission.
	 *
	...
	private function log_invalid_submission(
		...
		$message = wp_json_encode(
			[
				'event'                => 'courier_multi_newsletter_validation_blocked',
				'reason'               => $reason,
				'blog_id'              => get_current_blog_id(),
				'request_host'         => $this->request->getHost(),
				'signup_url_host'      => wp_parse_url( $signup_url, PHP_URL_HOST ),
				...
				'utm_source_platform'  => $utm_params['utm_source_platform'] ?? '',
				'utm_creative_format'  => $utm_params['utm_creative_format'] ?? '',
				'utm_marketing_tactic' => $utm_params['utm_marketing_tactic'] ?? '',
	...
	/**
	 * Get invalid submitted list names for a signup URL.
	 *
	 * @param string             $signup_url Submitted signup URL.
	 * @param array<int, string> $list_names Submitted list names.
	 * @return array<int, string>
	 */
	private function get_invalid_list_names_for_signup_url( string $signup_url, array $list_names ): array {
		$allowed_list_names = $this->get_allowed_list_names_for_signup_url( $signup_url );

		if ( count( $allowed_list_names ) === 0 ) {
			return $list_names;
		}

		return array_values( array_diff( $list_names, $allowed_list_names ) );
	}

	/**
	 * Get allowed list names from the signup URL content.
	 *
	 * @param string $signup_url Submitted signup URL.
	 * @return array<int, string>
	 */
	private function get_allowed_list_names_for_signup_url( string $signup_url ): array {
		...
		return $this->get_list_names_from_blocks( parse_blocks( $content ) );
	}

	/**
	 * Get post ID for a signup URL.
	 *
	 * @param string $signup_url Submitted signup URL.
	 * @return int
	 */
	...
	private function get_list_names_from_blocks( array $blocks, array $seen_refs = [] ): array {
		...
		foreach ( $blocks as $block ) {
			...
			if ( $block_name === 'courier/multi-newsletter-option' && isset( $attrs['listName'] ) && is_string( $attrs['listName'] ) ) {
				$list_names[] = sanitize_text_field( $attrs['listName'] );
			}

			if ( $block_name === 'core/block' && isset( $attrs['ref'] ) && is_numeric( $attrs['ref'] ) ) {
				$ref = (int) $attrs['ref'];

				if ( $ref > 0 && ! in_array( $ref, $seen_refs, true ) ) {
					$reusable_block = get_post( $ref );

					if ( $reusable_block instanceof WP_Post && is_string( $reusable_block->post_content ) ) {
						$list_names = array_merge(
							$list_names,
							$this->get_list_names_from_blocks( array_values( parse_blocks( $reusable_block->post_content ) ), array_merge( $seen_refs, [ $ref ] ) )
						);
					}
				}
			}
			...
			if ( isset( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) ) {
				$list_names = array_merge( $list_names, $this->get_list_names_from_blocks( array_values( $block['innerBlocks'] ), $seen_refs ) );
			}
		}
